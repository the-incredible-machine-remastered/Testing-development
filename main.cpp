...
Texture2D tex_robote_soporte;  // Soporte de BolaRebotadora
Texture2D tex_robote_pelota;   // Pelota roja de BolaRebotadora
// Texturas del ventilador
Texture2D tex_ventilador_cuerpo;
Texture2D tex_ventilador_aspa;
...
void dibujar_entidad(const EntidadFisica* e) {
    ...
    if (forma == TipoForma::CIRCULO) {
        ...
        if (vent) {
            ...
            // Aspas: dibujar texture rotada si existe, sino fallback con lineas
            if (tex_ventilador_aspa.id > 0) {
                float aspa_w = h * 0.5f + 20.0f; // longitud visual de la aspa
                float aspa_h = h * 0.3f - 20.0f; // grosor visual
                for (int i = 0; i < 4; ++i) {
...
void cargandoTexturas() {
    ...
    // Texturas del robot rojo (BolaRebotadora)
    tex_robote_soporte = load_tex("Assets/robote/proto-2.png");
    if (tex_robote_soporte.id == 0) {
        TraceLog(LOG_WARNING, "Textura robote soporte no encontrada: Assets/robote/proto-2.png");
    } else {
        TraceLog(LOG_INFO, "Textura robote soporte cargada: %dx%d", tex_robote_soporte.width, tex_robote_soporte.height);
    }
    tex_robote_pelota = load_tex("Assets/robote/proto-1.png");
    if (tex_robote_pelota.id == 0) {
        TraceLog(LOG_WARNING, "Textura robote pelota no encontrada: Assets/robote/proto-1.png");
    } else {
        TraceLog(LOG_INFO, "Textura robote pelota cargada: %dx%d", tex_robote_pelota.width, tex_robote_pelota.height);
    }

    // Texturas del ventilador (opcion A: cuerpo + aspa)
    ...
    // Cargar texturas de BolaRebotadora (robot rojo)
