...
bool spawn_desde_menu(MotorFisica& motor, TipoObjetoMenu tipo, Vector2D pos) {
    bool creado = false;
    switch (tipo) {
        case TipoObjetoMenu::BOLA:              creado = crear_bola(motor, pos); break;
        case TipoObjetoMenu::BOLA_REBOTADORA:   creado = crear_bola_rebotadora(motor, pos); break;
        case TipoObjetoMenu::TRAMPOLIN:         creado = crear_trampolin(motor, pos); break;
        case TipoObjetoMenu::BALANCIN:          creado = crear_balancin(motor, pos); break;
        case TipoObjetoMenu::RAMPA:             creado = crear_rampa(motor, pos, false); break;
        case TipoObjetoMenu::PLATAFORMA:        creado = crear_plataforma(motor, pos, 150.0, 15.0, TipoObjetoMenu::PLATAFORMA); break;
        case TipoObjetoMenu::PARED_LARGA:       creado = crear_plataforma(motor, pos, 80.0, 120.0, TipoObjetoMenu::PARED_LARGA); break;
        case TipoObjetoMenu::PLATAFORMA_DECOR:  creado = crear_plataforma(motor, pos, 120.0, 20.0, TipoObjetoMenu::PLATAFORMA_DECOR); break;
        case TipoObjetoMenu::SEGUIDOR_BOOSTER:  creado = crear_seguidor_booster(motor, pos); break;
        case TipoObjetoMenu::BARRIL_CHAVO:      creado = crear_barril_chavo(motor, pos); break;
        case TipoObjetoMenu::VENTILADOR:        creado = crear_ventilador(motor, pos); break;
        case TipoObjetoMenu::CUBETA:            creado = crear_cubeta(motor, pos); break;
        case TipoObjetoMenu::SOPORTE_TORQUE:    creado = crear_soporte_torque(motor, pos); break;
        case TipoObjetoMenu::GANCHO:            creado = crear_gancho(motor, pos); break;
        case TipoObjetoMenu::PISTOLA:           creado = crear_pistola(motor, pos); break;
        case TipoObjetoMenu::GLOBO:             creado = crear_globo(motor, pos); break;
        case TipoObjetoMenu::BOLA_BEISBOL:      creado = crear_bola_beisbol(motor, pos); break;
        case TipoObjetoMenu::TIJERA:            creado = crear_tijera(motor, pos); break;
        case TipoObjetoMenu::ZONA_META:         creado = crear_zona_meta(motor, pos); break;
        case TipoObjetoMenu::CUERDA:            creado = false; break;
        default: creado = false; break;
    }
    if (creado) {
        movimientos_usuario++;
    }
    ...
    switch (tipo) {
        case TipoObjetoMenu::BOLA: {
            ...
            if (tex_bola[0].id > 0) {
                DrawTexturePro(
                    tex_bola[0],
                    {0.0f, 0.0f, (float)tex_bola[0].width, (float)tex_bola[0].height},
