...
public:
    ...
    Vector2D calcular_aceleracion_en(const Vector2D& pos, const Vector2D& vel) const override {
        ...
        // Flotación negativa (hacia arriba) compite con gravedad
        // fuerza_neta ya incluye gravedad aplicada por el motor
    ...
    void on_collision(EntidadFisica* otro, const InfoColision& info) override {
        // Si choca con algo arriba, cancelar velocidad vertical para presionar
        if (info.normal.y > 0.3) {
            Vector2D vel = get_velocidad();
            set_velocidad(Vector2D(vel.x * 0.3, 0.0));
        }

        // Si choca con un balancín, aplicar torque en el extremo más cercano
        Balancin* bal = dynamic_cast<Balancin*>(otro);
        if (bal) {
            Vector2D ext_izq = bal->get_punto_extremo_izquierdo();
            Vector2D ext_der = bal->get_punto_extremo_derecho();
            double dist_izq = (posicion - ext_izq).magnitud();
            double dist_der = (posicion - ext_der).magnitud();
